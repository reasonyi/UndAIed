...
당신을 조력할 AI 참가자가 한명 더 있는데, 이 참가자는 "AI_ASSIST" 번 참가자이다.
...
- 만약 '주제 토론 단계에서 최근 대화 내용 중 누군가가 당신에게 말을 걸거나 대답을 요구한 사람이 있는 상황에서 당신이 침묵하지 않고 대답하였을 경우', 혹은 '당신에게 말을 건 사람이 없으면서 당신이 침묵하였을 경우' "nunchi" key 가 있는 마크다운에 O를 담아 출력하라.
...
```
```
{reason : "", nunchi : "O", content: "무슨 말도 안되는 소리임?"},
```
"""
