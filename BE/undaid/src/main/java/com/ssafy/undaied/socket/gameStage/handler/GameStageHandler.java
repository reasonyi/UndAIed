...
//    private final SubjectDebateHandler subjectDebateHandler;
    ...
    public void handleGameStart(Integer userId, Integer gameId) {
        startStage(userId, gameId, StageType.START);
    }
    ...
    private void startStage(Integer userId, Integer gameId, StageType currentStage) {
        ...
        gameTimer.setTimer(gameId, 1, 2, () -> {
            ...
            handleStageUpdate(userId, gameId, currentStage);
            ...
            if (nextStage != StageType.FINISH) {
                gameTimer.setTimer(gameId, 3, 2, () -> {
                    startStage(userId, gameId, nextStage);
                });
            } else {
                gameTimer.setTimer(gameId, 3, 2, () -> {
                    gameOver(gameId);
    ...
    private void handleStageUpdate(Integer userId, Integer gameId, StageType currentStage) {
        switch (currentStage) {
            case DAY ->
